...
4. 播放时应先请求应用内的 `master.m3u8` 和子播放列表。后端会修正腾讯云
   偶尔生成的 `BANDWIDTH=0`，避免 Apple 原生 HLS 拒绝播放。一般浏览器的
   TS 分片请求会收到 307 跳转并直接从 COS 下载；iPhone、iPad 和 Safari
   的分片由应用同源转发，以避开 COS 默认域名强制下载响应导致的原生播放
   失败。
